...
## 10. 基于现状的前端页面简要方案（AI Native + 直接操控）

> 目标：在保留运营同学“快操作、快反馈”的前提下，让 AI 成为默认交互入口，并补齐“过程可视化页面”。

### 10.1 全局信息架构（建议）

按“目标 -> 执行 -> 观察 -> 优化”组织导航：

1. **总览（Home）**：今日目标、风险提醒、待处理事项。  
2. **增长运营（Campaigns / Journeys）**：活动与自动化流程编排。  
3. **线索与客户（Leads / Conversations）**：线索池、会话、分层与跟进。  
4. **AI 控制台（AI Studio）**：提示词、工具、路由策略、知识库。  
5. **分析与归因（Analytics / ROI）**：效果、成本、归因与建议动作。  
6. **过程中心（Ops Timeline）**：任务执行过程、失败重试、人工接管。  
7. **系统设置（Settings）**：渠道、CRM、权限、组织配置。
...
- 展示北极星指标、关键漏斗、异常告警。
- AI 给出“今日 3 个最值得做的动作”。
- 一键进入对应执行页（如“创建挽回活动”“修复失败同步”）。

#### B. 活动页（Campaigns）
- 活动列表、状态、发送规模、核心转化结果。
- 支持“AI 生成活动草案”（人群 + 文案 + 发送时间）。
- 支持快速动作：暂停、恢复、复制、A/B 小实验。

#### C. 旅程页（Journeys）
- 可视化流程画布（触发、条件、延时、发送、Webhook）。
- AI 协助补全分支逻辑（如“高意向线索自动转人工”）。
- 提供“模拟运行”与“节点级结果预览”。

#### D. 线索与会话页（Leads / Conversations）
- 线索列表 + 分群筛选 + 阶段看板。
- 单线索时间线：消息、标签、AI 判断、人工操作记录。
- 支持“AI 下一步建议”与“一键执行建议动作”。

#### E. AI Studio（Prompts / Tools / Routing / Knowledge）
- 统一管理 Prompt、工具权限、路由策略、知识源。
- 支持版本管理与灰度发布（小流量验证）。
- 提供在线测试面板：输入上下文即看 AI 决策与调用链。

#### F. 分析归因页（Analytics / ROI）
- 从渠道、活动、旅程、线索阶段多维查看结果。
- 内置归因模型切换（first/last/linear）与差异对比。
- AI 自动生成“问题诊断 + 下周优化建议”。

#### G. 过程中心页（Ops Timeline）
- 展示从入站消息到任务执行的全链路事件流。
- 可查看队列状态、失败原因、重试历史、人工接管点。
- 支持“直接操作”：重试、跳过、回滚、转人工。

#### H. 设置页（Settings）
- 渠道接入（如 MessageBird）、CRM 映射、Webhook、组织权限。
- 配置变更有“影响预览”与“回滚入口”。

### 10.3 AI Native 交互原则（轻量）

1. **先给建议，再给按钮**：每个核心页面都应有 AI 建议卡片与可执行按钮。  
2. **可解释可回放**：关键 AI 决策都能查看依据、输入上下文与执行日志。  
3. **人机协同闭环**：支持“AI 自动执行 / 人工确认后执行 / 完全人工”三档模式。  
4. **过程可见优先于结果可见**：不仅看结果图表，还能看到每一步如何发生。

### 10.4 “便捷直接操控”最小能力包

- 全局命令面板（快速跳转、快速创建、快速执行）。
- 统一“快速操作区”（暂停/重试/转人工/加白名单）。
- 关键流程都提供“模拟运行”与“一键回放”。
- 页面内置 AI 对话侧栏，支持“问原因、要建议、直接执行”。
