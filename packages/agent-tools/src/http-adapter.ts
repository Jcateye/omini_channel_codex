...
const normalizeRetryConfig = (value: unknown) => {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    return {
...
const getPathValue = (input: unknown, path: string) => {
...
const normalizeAuth = (tool: ToolDefinition) => {
  if (!tool.auth || typeof tool.auth !== 'object' || Array.isArray(tool.auth)) {
    return { scheme: 'none' as const, secretRef: '' };
  }
...
const resolveSecret = (secretRef: string) => {
  if (!secretRef) return '';
...
export const httpExternalAdapter: ExternalToolAdapter = {
  id: 'external.http',
  name: 'HTTP Tool Adapter',
  provider: 'http',
  healthcheck: async () => ({ status: 'ok' }),
  execute: async (tool, request): Promise<ToolExecutionResult> => {
    ...
    const outputMap =
      config.outputMap && typeof config.outputMap === 'object' && !Array.isArray(config.outputMap)
        ? (config.outputMap as Record<string, unknown>)
        : null;
    ...
    const body =
      canSendBody || allowBody ? JSON.stringify(payload ?? {}) : undefined;
      ...
      try {
        ...
        if (!response.ok) {
          const errorFromPayload =
            parsed && typeof parsed === 'object' && errorPath
