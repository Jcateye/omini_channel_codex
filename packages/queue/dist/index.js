...
export const createQueue = (name) => queueFactory.createQueue(name);
export const createWorker = (name, handlerOrConcurrency, concurrency = 1) => {
    const resolvedConcurrency = typeof handlerOrConcurrency === 'number' ? handlerOrConcurrency : concurrency;
    const worker = queueFactory.createWorker(name, resolvedConcurrency);
