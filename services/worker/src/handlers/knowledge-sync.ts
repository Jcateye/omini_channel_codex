...
type KnowledgeSyncJob =
  | { type: 'sync-source'; syncId: string }
  | { type: 'embed-chunk'; chunkId: string };
...
const deleteQdrantBySource = async (input: {
  ...
  await qdrantFetch(
    input.settings,
    `/collections/${input.settings.collection}/points/delete?wait=true`,
    {
      method: 'POST',
...
const upsertQdrantPoints = async (input: {
  ...
  await qdrantFetch(
    input.settings,
    `/collections/${input.settings.collection}/points?wait=true`,
    {
      method: 'PUT',
...
const crawlWebSource = async (config: Record<string, unknown>) => {
  ...
  const crawlDepth =
    typeof config.crawlDepth === 'number' && config.crawlDepth >= 0
...
const notionFetch = async (token: string, path: string) => {
...
const extractNotionText = (block: Record<string, unknown>) => {
...
const collectNotionBlocks = async (
  token: string,
  blockId: string,
  depth: number,
  maxDepth: number,
...
const loadNotionDocuments = async (config: Record<string, unknown>) => {
  const token = typeof config.token === 'string' ? config.token : '';
  const pageIds = Array.isArray(config.pageIds)
    ? config.pageIds.filter((id) => typeof id === 'string')
    : [];
  const maxDepth =
    typeof config.maxDepth === 'number' && config.maxDepth > 0 ? Math.floor(config.maxDepth) : 2;
...
const loadGoogleDocs = async (config: Record<string, unknown>) => {
  ...
  for (const docId of documentIds) {
    ...
    const body = payload.body as { content?: Array<Record<string, unknown>> } | undefined;
    const content = Array.isArray(body?.content) ? body?.content : [];
    const lines: string[] = [];

    for (const item of content) {
      if (!item || typeof item !== 'object') continue;
      const paragraph = (item as Record<string, unknown>).paragraph as
...
const ingestKnowledgeSource = async (input: {
  ...
  const chunkSize =
    input.config && typeof input.config === 'object' && !Array.isArray(input.config)
      ? (input.config as Record<string, unknown>).chunkSize
      : null;
  const maxChunkSize =
    typeof chunkSize === 'number' && chunkSize > 0 ? Math.floor(chunkSize) : 600;
...
const handleSyncSource = async (syncId: string) => {
  ...
  let stats: { documents: number; chunks: number } | null = null;
...
const scheduleDueKnowledgeSyncs = async () => {
  ...
  for (const source of sources) {
    ...
    const intervalMinutes =
      typeof config.syncIntervalMinutes === 'number' && config.syncIntervalMinutes > 0
...
export const registerKnowledgeSyncWorker = () =>
  createWorker<KnowledgeSyncJob>(QUEUE_NAMES.knowledgeSync, async ({ data }) => {
    if (!data) {
      return;
    }
...
export const startKnowledgeSyncScheduler = () => {
  const intervalMs =
    typeof process.env.KNOWLEDGE_SYNC_POLL_MS === 'string'
