...
const insightQueue = createQueue(QUEUE_NAMES.aiInsights);
...
const processWindow = async (windowStart, windowEnd) => {
...
export const registerAiInsightsWorker = () => createWorker(QUEUE_NAMES.aiInsights, async ({ data }) => {
    const now = new Date();
...
export const startAiInsightsScheduler = () => {
    ...
    const tick = async () => {
        const now = new Date();
        const { windowStart } = resolveWindow(now);
        await insightQueue.add('ai.window', { windowStart: windowStart.toISOString() }, defaultJobOptions);
    };
