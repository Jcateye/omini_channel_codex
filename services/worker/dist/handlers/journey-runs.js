...
const selectEdgesForCondition = (edges, outcome) => {
...
const createRunSteps = async (input) => {
    ...
    const steps = await prisma.$transaction(async (tx) => {
        const created = await Promise.all(input.nodes.map((node) => tx.journeyRunStep.create({
            data: {
                organizationId: input.organizationId,
                runId: input.runId,
                nodeId: node.id,
...
const handleTriggerJob = async (job) => {
    ...
    for (const trigger of triggerRecords) {
        ...
        const matches = matchesTrigger(trigger.config, {
            tags,
            stage,
            text: job.text ?? null,
        });
        ...
        await startJourneyRun({
            ...
            context: (() => {
                ...
                if (job.channelId)
                    context.channelId = job.channelId;
                ...
                if (job.messageId)
                    context.messageId = job.messageId;
...
const handleStepJob = async (job) => {
    ...
    const triggerPayload = step.run.triggerPayload && typeof step.run.triggerPayload === 'object'
        ? step.run.triggerPayload
        : {};
    const context = {
        tags: lead?.tags ?? [],
    ...
    try {
        if (step.node.type === 'delay') {
            const config = step.node.config && typeof step.node.config === 'object' && !Array.isArray(step.node.config)
                ? step.node.config
                : {};
            ...
            const updatedTags = Array.from(new Set(lead.tags
            ...
            const channelId = typeof config.channelId === 'string' && config.channelId.trim().length > 0
...
export const registerJourneyRunsWorker = () => createWorker(QUEUE_NAMES.journeyRuns, async ({ data }) => {
    if (!data) {
...
export const enqueueJourneyTrigger = async (job) => {
    await journeyQueue.add('journey.trigger', job, defaultJobOptions);
};
//# sourceMappingURL=journey-runs.js.map