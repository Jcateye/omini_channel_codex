...
const toNullableJson = (value) => value == null ? Prisma.DbNull : value;
...
const normalizeAgentRoutingConfig = (input) => {
...
const normalizeLeadRules = (input) => {
...
const normalizeTagSet = (tags) => Array.from(new Set(tags.map((tag) => tag.trim()).filter((tag) => tag.length > 0)))
    .sort()
    .join('|');
const enqueueJourneySignal = async (input) => {
    await journeyQueue.add('journey.trigger', {
        type: 'trigger',
...
const normalizeJourneyStatus = (input) => {
    const value = typeof input === 'string' ? input.trim() : '';
    return journeyStatuses.has(value) ? value : 'draft';
};
const normalizeJourneyTriggers = (input) => {
    if (!Array.isArray(input)) {
        return [];
    }
    return input
...
const normalizeJourneyNodes = (input) => {
    if (!Array.isArray(input)) {
        return [];
    }
    return input
...
const normalizeJourneyEdges = (input) => {
    if (!Array.isArray(input)) {
        return [];
    }
    return input
...
const safeRate = (numerator, denominator) => denominator > 0 ? Number((numerator / denominator).toFixed(4)) : 0;
const clampNumber = (value, min, max) => Math.min(max, Math.max(min, value));
const normalizeAnalyticsSettings = (input) => {
    if (!input || typeof input !== 'object') {
        return {
    ...
    const attributionLookbackDaysRaw = typeof settings.attributionLookbackDays === 'number'
        ? settings.attributionLookbackDays
        : 7;
    const aggregationDaysRaw = typeof settings.aggregationDays === 'number' ? settings.aggregationDays : 30;
    const realtimeWindowMinutesRaw = typeof settings.realtimeWindowMinutes === 'number' ? settings.realtimeWindowMinutes : 60;
    return {
...
const normalizeAgentIntelligenceSettings = (input) => {
    if (!input || typeof input !== 'object') {
        return {
    ...
    const settings = input;
    const memoryRetentionDays = typeof settings.memoryRetentionDays === 'number' && settings.memoryRetentionDays > 0
...
const normalizeDistributionStrategies = (input) => {
    const defaults = {
        mode: 'round_robin',
        targets: [
    ...
    const raw = input;
    const mode = typeof raw.mode === 'string' && ['round_robin', 'weighted', 'skill_based'].includes(raw.mode)
        ? raw.mode
        : defaults.mode;
...
const normalizeAgentHandoffConfig = (input) => {
    ...
    const roles = Array.isArray(raw.roles)
        ...
        : defaults.roles;
    const stageRoles = raw.stageRoles && typeof raw.stageRoles === 'object' && !Array.isArray(raw.stageRoles)
        ? Object.entries(raw.stageRoles).reduce((acc, [key, value]) => {
    ...
    const rules = Array.isArray(raw.rules)
        ? raw.rules
            ...
            const minConfidence = typeof item.minConfidence === 'number' ? item.minConfidence : undefined;
...
const applyContextAllowlist = (source, allowlist) => {
    const result = {};
...
const evaluateHandoff = (input) => {
    ...
    const currentRole = typeof input.lead.metadata?.agentRole === 'string'
        ? input.lead.metadata.agentRole
    ...
    for (const rule of input.config.rules) {
        if (!rule.enabled)
            continue;
        if (!rule.targetRole || rule.targetRole === currentRole)
            continue;
...
const selectDistributionTarget = (input) => {
    ...
    if (distribution.mode === 'skill_based') {
        const matched = targets.find((target) => {
            ...
            const skillMatch = skills.length === 0 || skills.some((skill) => leadTags.includes(skill));
...
const normalizeQueryTokens = (value) => value
    .toLowerCase()
    .split(/\s+/)
...
const searchQdrant = async (input) => {
    ...
    if (input.sourceIds && input.sourceIds.length > 0) {
        filter.must.push({
            key: 'sourceId',
            match: input.sourceIds.length === 1
                ? { value: input.sourceIds[0] }
                : { any: input.sourceIds },
        });
    }
    const payload = await qdrantFetch(input.settings, `/collections/${input.settings.collection}/points/search`, {
        method: 'POST',
        body: JSON.stringify({
            vector: input.vector,
            limit: input.topK,
...
const chunkMatchesTags = (metadata, tags) => {
    ...
    const normalized = value.filter((tag) => typeof tag === 'string');
...
const retrieveKnowledgeChunksKeyword = async (input) => {
...
const retrieveKnowledgeChunksVector = async (input) => {
    const query = input.query.trim();
    if (!query) {
        return [];
    }
...
const normalizeToolSchema = (input) => {
    if (!input || typeof input !== 'object') {
        return { input: {}, output: {} };
    }
    const schema = input;
    const inputSchema = schema.input && typeof schema.input === 'object' && !Array.isArray(schema.input)
        ? schema.input
        : {};
    const outputSchema = schema.output && typeof schema.output === 'object' && !Array.isArray(schema.output)
        ? schema.output
        : {};
    return { input: inputSchema, output: outputSchema };
};
const normalizeToolDefinitionInput = (input) => {
    ...
    const config = input.config && typeof input.config === 'object' && !Array.isArray(input.config)
        ? input.config
        : null;
    const auth = input.auth && typeof input.auth === 'object' && !Array.isArray(input.auth)
        ? input.auth
        : null;
...
const normalizePermissionInput = (input) => {
...
const checkToolPermission = async (organizationId, toolId, agentId) => {
...
const normalizeLeadStage = (stage) => typeof stage === 'string' && leadStages.has(stage) ? stage : undefined;
...
const applyLeadUpdate = async (lead, updates) => {
    ...
    if ('metadata' in conversionUpdates) {
        data.metadata = toNullableJson(conversionUpdates.metadata &&
...
const computeLeadAgentDecision = (input) => {
    ...
    const hasSignal = (value) => signals.includes(value);
    ...
    if (assignmentQueue) {
        const existingMetadata = input.lead.metadata && typeof input.lead.metadata === 'object' && !Array.isArray(input.lead.metadata)
            ? { ...input.lead.metadata }
            : {};
...
const runLeadAgentWorkflow = async (input) => {
    ...
    const distributionSettings = normalizeDistributionStrategies(settings.agentStrategies?.distribution);
    ...
    const memoryInputs = memories.map((memory) => ({
        content: memory.content && typeof memory.content === 'object' && !Array.isArray(memory.content)
            ? memory.content
            : {},
    }));
    ...
    const derivedStage = typeof decision.updates.stage === 'string' ? decision.updates.stage : lead.stage;
    const derivedTags = Array.isArray(decision.updates.tags)
        ? decision.updates.tags
        : lead.tags;
    ...
    if (distributionDecision.nextCursor !== undefined) {
        const agentStrategies = settings.agentStrategies && typeof settings.agentStrategies === 'object'
            ? { ...settings.agentStrategies }
            : {};
    ...
    const nextLeadStage = typeof mergedUpdates.stage === 'string' ? mergedUpdates.stage : lead.stage;
    const nextLeadTags = Array.isArray(mergedUpdates.tags) ? mergedUpdates.tags : lead.tags;
    const nextLeadScore = typeof mergedUpdates.score === 'number' ? mergedUpdates.score : lead.score ?? null;
    const nextLeadSource = typeof mergedUpdates.source === 'string' ? mergedUpdates.source : lead.source ?? null;
    const currentRole = typeof baseMetadata.agentRole === 'string'
        ? baseMetadata.agentRole
...
const normalizeCrmFieldMapping = (input) => {
    if (!input || typeof input !== 'object') {
        return {};
    }
    const mapping = input;
...
const validateCrmFieldMapping = (mapping) => {
    const errors = [];
...
const applyCrmMetadataMapping = (metadata, payload, mapping) => {
...
const normalizeCampaignSegment = (input) => {
    if (!input || typeof input !== 'object') {
        return {
            stages: [],
            tagsAll: [],
    ...
    const lastActiveWithinDays = typeof segment.lastActiveWithinDays === 'number' && segment.lastActiveWithinDays > 0
        ? Math.floor(segment.lastActiveWithinDays)
        : null;
    return { stages, tagsAll, sources, lastActiveWithinDays };
};
const buildSegmentWhere = (organizationId, segment) => {
    const where = {
        organizationId,
        ...(segment.stages.length > 0
            ? { stage: { in: segment.stages } }
            : {}),
...
const resolveWebhookSecret = (channel) => {
    const credentials = channel.credentials && typeof channel.credentials === 'object' && !Array.isArray(channel.credentials)
        ? channel.credentials
        : null;
    const settings = channel.settings && typeof channel.settings === 'object' && !Array.isArray(channel.settings)
        ? channel.settings
        : null;
    const credentialSecret = typeof credentials?.webhookSecret === 'string' ? credentials.webhookSecret.trim() : '';
    if (credentialSecret)
        return credentialSecret;
    const settingsSecret = typeof settings?.webhookSecret === 'string' ? settings.webhookSecret.trim() : '';
    if (settingsSecret)
        return settingsSecret;
    const envSecret = process.env.WEBHOOK_SIGNING_SECRET;
    return envSecret ? envSecret.trim() : '';
};
const parseWebhookTimestamp = (raw) => {
    if (!raw)
        return null;
...
const buildWebhookSignature = (secret, timestamp, rawBody) => crypto.createHmac('sha256', secret).update(`${timestamp}.${rawBody}`).digest('hex');
...
app.post('/v1/webhooks/whatsapp/:provider/:channelId', async (c) => {
    ...
    await inboundQueue.add('wa.webhook.live', {
        channelId,
        payload,
        rawBody,
        headers,
...
app.post('/v1/webhooks/whatsapp/:provider/:channelId/status', async (c) => {
    ...
    const headers = Object.fromEntries(c.req.raw.headers.entries());
    await statusQueue.add('wa.status', {
        channelId,
        payload,
        rawBody,
        headers,
...
api.put('/v1/agent/settings', async (c) => {
    const body = await readJsonBody(c);
    const agentSettings = normalizeAgentIntelligenceSettings(body.agentIntelligence ?? body);
    const settings = await loadOrganizationSettings(c.get('tenantId'));
    await prisma.organization.update({
        where: { id: c.get('tenantId') },
        data: {
...
api.get('/v1/agent/strategies', async (c) => {
    const settings = await loadOrganizationSettings(c.get('tenantId'));
    const optimization = normalizeOptimizationStrategies(settings.agentStrategies?.optimization);
    const distribution = normalizeDistributionStrategies(settings.agentStrategies?.distribution);
    return c.json({ strategies: { optimization, distribution } });
});
api.put('/v1/agent/strategies', async (c) => {
    const body = await readJsonBody(c);
    const payload = body.strategies ?? body;
...
api.post('/v1/agent/handoffs/preview', async (c) => {
    ...
    const confidence = typeof body.confidence === 'number' && body.confidence >= 0 ? body.confidence : null;
    const settings = await loadOrganizationSettings(c.get('tenantId'));
    const config = normalizeAgentHandoffConfig(settings.agentHandoffs);
    let leadStage = stage;
    let leadTags = tags;
...
api.get('/v1/agent/optimizations/preview', async (c) => {
    const settings = await loadOrganizationSettings(c.get('tenantId'));
    const strategies = normalizeOptimizationStrategies(settings.agentStrategies?.optimization);
...
api.post('/v1/knowledge-sources/:id/chunks', async (c) => {
    ...
    const chunkSize = typeof body.chunkSize === 'number' && body.chunkSize > 0 ? Math.floor(body.chunkSize) : 600;
    ...
    await Promise.all(chunkRows.map((row) => knowledgeQueue.add('embed-chunk', { type: 'embed-chunk', chunkId: row.id }, defaultJobOptions)));
...
api.get('/v1/knowledge-sources/:id/syncs', async (c) => {
    ...
    const limit = typeof c.req.query('limit') === 'string' ? Number(c.req.query('limit')) : 20;
...
api.post('/v1/knowledge/retrieve', async (c) => {
    ...
    const tags = Array.isArray(body.tags)
        ...
        : undefined;
    const recentDays = typeof body.recentDays === 'number' && body.recentDays > 0 ? body.recentDays : null;
    const since = typeof body.since === 'string' && body.since.trim().length > 0
        ? new Date(body.since)
        : null;
    const minCreatedAt = since && !Number.isNaN(since.getTime())
        ? since
        : recentDays
...
api.get('/v1/analytics/campaigns', async (c) => {
    ...
    const metrics = rows.map((row) => {
        ...
        const roi = typeof cost === 'number' && cost > 0 && typeof revenue === 'number'
...
api.get('/v1/analytics/trends/channels', async (c) => {
    ...
    const channelIds = Array.from(new Set(rows.map((row) => row.channelId).filter((value) => !!value)));
    const channels = channelIds.length
        ? await prisma.channel.findMany({
            where: { id: { in: channelIds }, organizationId: c.get('tenantId') },
            select: { id: true, name: true, platform: true, provider: true },
...
const resolveInsightWindow = async (organizationId, windowRaw, model) => {
...
api.get('/v1/insights/intents', async (c) => {
    const windowStart = await resolveInsightWindow(c.get('tenantId'), c.req.query('windowStart'), 'intent');
    if (!windowStart) {
...
api.get('/v1/insights/clusters', async (c) => {
    const windowStart = await resolveInsightWindow(c.get('tenantId'), c.req.query('windowStart'), 'cluster');
    if (!windowStart) {
...
api.get('/v1/insights/suggestions', async (c) => {
    const windowStart = await resolveInsightWindow(c.get('tenantId'), c.req.query('windowStart'), 'suggestion');
    if (!windowStart) {
...
api.get('/v1/attribution/report', async (c) => {
    ...
    const model = typeof modelRaw === 'string' && attributionModels.has(modelRaw)
        ? modelRaw
        : 'last_touch';
    const { start, end } = resolveDateRange(c.req.query('start'), c.req.query('end'));
    ...
    const sumBy = (items) => items.reduce((total, item) => total + item.weight, 0);
    const groupBy = (items) => {
        const map = new Map();
    ...
    const channelMap = groupBy(touchpoints.map((item) => ({ key: item.channelId, weight: item.weight })));
    const campaignMap = groupBy(touchpoints.map((item) => ({ key: item.campaignId, weight: item.weight })));
    const journeyMap = groupBy(touchpoints.map((item) => ({ key: item.journeyId, weight: item.weight })));
...
api.get('/v1/agent/optimizations', async (c) => {
    ...
    const status = typeof statusRaw === 'string' && campaignOptimizationStatuses.has(statusRaw)
        ? statusRaw
        : undefined;
    const where = {
        organizationId: c.get('tenantId'),
...
api.post('/v1/agent/optimizations/:id/apply', async (c) => {
    ...
    const campaignMetadata = optimization.campaign.metadata && typeof optimization.campaign.metadata === 'object' && !Array.isArray(optimization.campaign.metadata)
        ? { ...optimization.campaign.metadata }
        : {};
...
api.get('/v1/agent/runs', async (c) => {
    ...
    const leadId = c.req.query('leadId');
    const campaignId = c.req.query('campaignId');
    const type = typeof typeRaw === 'string' && agentRunTypes.has(typeRaw)
        ? typeRaw
        : undefined;
    const where = {
        organizationId: c.get('tenantId'),
...
api.post('/v1/agent-tools/:id/execute', async (c) => {
    ...
    if (tool.kind === 'external') {
        const adapterId = tool.config && typeof tool.config === 'object'
            ? tool.config.adapterId
            : null;
        ...
        if (!adapter) {
            ...
            result = await adapter.execute(tool, {
                toolId: tool.id,
                agentId,
                inputs,
                context,
...
api.post('/v1/crm/leads/:id', async (c) => {
    ...
    if (fieldMapping && Object.keys(fieldMapping).length > 0) {
        const existingMetadata = updates.metadata && typeof updates.metadata === 'object' && !Array.isArray(updates.metadata)
            ? updates.metadata
            : lead.metadata && typeof lead.metadata === 'object' && !Array.isArray(lead.metadata)
                ? lead.metadata
                : null;
...
api.post('/v1/crm/revenue', async (c) => {
    ...
    const occurredAt = typeof occurredAtRaw === 'string' || typeof occurredAtRaw === 'number'
    ...
    const revenueEvent = await prisma.revenueEvent.create({
        data: {
            ...
            externalId,
            occurredAt,
            metadata: toNullableJson(body.metadata && typeof body.metadata === 'object' && !Array.isArray(body.metadata)
                ? body.metadata
                : null),
        },
    });
...
api.put('/v1/crm/webhook', async (c) => {
    ...
    if (Array.isArray(body.events)) {
        webhook.events = body.events.filter((event) => typeof event === 'string');
    }
    const updated = {
        ...settings,
        crmWebhook: webhook,
...
api.post('/v1/leads/:id/signals', async (c) => {
    ...
    const confidence = typeof body.confidence === 'number' && body.confidence >= 0 ? body.confidence : undefined;
    ...
    if (leadRules.length > 0) {
        ...
        ruleResult = applyLeadRules({
            ...
            metadata: updatedLead.metadata && typeof updatedLead.metadata === 'object' && !Array.isArray(updatedLead.metadata)
                ? updatedLead.metadata
                : null,
        }, leadRules, leadRuleContext);
        if (Object.keys(ruleResult.updates).length > 0) {
            ...
            if ('metadata' in ruleUpdates) {
                leadUpdate.metadata = toNullableJson(ruleUpdates.metadata &&
    ...
    if (Object.keys(combinedUpdates).length > 0) {
        await enqueueCrmWebhook(c.get('tenantId'), 'lead.updated', {
            lead: updatedLead,
...
api.post('/v1/agent/leads/:id/score', async (c) => {
    ...
    const tagsChanged = normalizeTagSet(agentResult.lead.tags ?? []) !== normalizeTagSet(originalTags);
...
api.put('/v1/journeys/:id', async (c) => {
    ...
    const description = typeof body.description === 'string' ? body.description.trim() : journey.description;
...
api.post('/v1/whatsapp/channels/:channelId/messages', async (c) => {
    ...
    await outboundQueue.add('wa.send', {
...
api.post('/v1/mock/whatsapp/inbound', async (c) => {
    ...
    await inboundQueue.add('wa.webhook.mock', {
        channelId,
        payload,
...
admin.post('/v1/admin/bootstrap', async (c) => {
    ...
    const apiKeyName = body.apiKeyName ?? 'default';
    if (!name || !slug) {
        return c.json({ error: 'invalid_payload' }, 400);
    }
